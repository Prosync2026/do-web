...
export default defineComponent({
    ...
    setup(props, { emit }) {
            ...
            (newItem) => {
                if (newItem) {
                    const filled = {
                        ...
                        Element: newItem.element ?? newItem.elementCode ?? '',
                        SubElement: newItem.subElement ?? '',
                        SubSubElement: newItem.subSubElement ?? '',
                        Unit: newItem.unit ?? '',
                        Quantity: newItem.latestQty ?? newItem.qty ?? 0,
                        Rate: newItem.latestRate ?? newItem.rate ?? 0,
        ...
        const submit = async () => {
            ...
            // handle both camelCase, PascalCase, and specific comparison IDs to be safe
            const itemId = props.item?.budgetItemId ?? props.item?.id ?? props.item?.Id;
            if (!itemId) {
