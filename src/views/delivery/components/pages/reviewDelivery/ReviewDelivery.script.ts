...
import { ProCard, ProButton, ProTag, ProInput, ProUploadFile } from '@prosync_solutions/ui';
import { PhCheck, PhTrash, PhPlus, PhCamera, PhFilePdf } from '@phosphor-icons/vue';

export default defineComponent({
    name: 'ReviewDelivery',
    components: { ProCard, ProButton, ProTag, ProInput, ProUploadFile, PhCheck, PhTrash, PhPlus, PhCamera, PhFilePdf },
    setup() {
        ...
        const evidenceAttachments = ref<any[]>([]);
        ...
        const handleSubmit = () => {
            if (deliveryOrder.value) {
                const docIndex = deliveryStore.list.findIndex((d: any) => d.Id === deliveryOrderId);
                if (docIndex !== -1) {
                    (deliveryStore.list[docIndex] as any)._aiExtractedItems = extractedItems.value;
                    (deliveryStore.list[docIndex] as any)._evidenceAttachments = evidenceAttachments.value;
                    deliveryStore.list[docIndex].Status = 'Pending';
                    
                    toast.add({
                        severity: 'success',
                        summary: 'Submitted',
                        detail: `Delivery order ${deliveryOrder.value.DocNo} has been submitted.`,
                        life: 3000
                    });
                }
            }
